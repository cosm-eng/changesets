...
- status [--since --verbose --output=JSON_FILE.json]
- pre [exit|enter {tag}]
- tag
...
- `--since` - will compare the current branch instead of relying on the baseBranch attribute from the config file, this is can be used if you are using multiple branches and need to compare with different branches based on the base
