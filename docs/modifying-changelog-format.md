...
For example, `changesets` has a package, `@cosm-changesets/changelog-git`. To use it, you would first need to install the package.

```
yarn add @cosm-changesets/changelog-git
```

Next, change your `.changeset/config.json` to point to the new package:

```
"changelog": "@cosm-changesets/changelog-git"
```
...
If you are using typescript to write your changelog functions, you can use the type. First install `@cosm-changesets/types`, and then:

```ts
import { ChangelogFunctions } from "@cosm-changesets/types";
...
type getReleaseLine(
    ...
    type
    // This needs to be explained - see @cosm-changesets/changelog-github's code for how this works
    changelogOpts
) => string
```
