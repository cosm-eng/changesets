...
import { ValidationError } from "@cosm-changesets/errors";
...
export let defaultWrittenConfig = {
  $schema: `https://unpkg.com/@cosm-changesets/config@${packageJson.version}/schema.json`,
...
function getNormalizedCommitOption(
  ...
  if (thing === true) {
    return ["@cosm-changesets/cli/commit", { skipCI: "version" }];
  }
  if (typeof thing === "string") {
    return [thing, null];
  }
...
export let parse = (json: WrittenConfig, packages: Packages): Config => {
  ...
  if (
    ...
    messages.push(
      `The \`commit\` option is set as ${JSON.stringify(
        ...
        null,
        2
      )} when the only valid values are undefined or a boolean or a module path (e.g. "@cosm-changesets/cli/commit" or "./some-module") or a tuple with a module path and config for the commit message generator (e.g. ["@cosm-changesets/cli/commit", { "skipCI": "version" }])`
    );
  }
  ...
  if (
    ...
    !["patch", "minor", "none"].includes(json.updateInternalDependencies)
  ) {
    messages.push(
      `The \`updateInternalDependencies\` option is set as ${JSON.stringify(
        ...
        null,
        2
      )} but can only be 'patch', 'minor', or 'none'`
    );
  }
  if (json.ignore) {
    if (
